...
const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-slate-700 p-3 border border-slate-600 rounded-lg shadow-xl text-sm w-48">
          <p className="font-bold text-slate-200 mb-2">{`Cell #${label}`}</p>
          <p className="text-cyan-300">{`Voltage: ${payload[0].value.toFixed(3)} V`}</p>
          <p className="text-amber-400">{`Resistance: ${(payload[1].value * 1000).toFixed(1)} mΩ`}</p>
          <p className="text-slate-400">{`Z-score: ${cellData.zScore.toFixed(2)}`}</p>
          
          {cellData.isAnomaly && (
            <div className="mt-2 pt-2 border-t border-slate-600">
                <p className="font-semibold text-red-400">Status: Anomaly Detected</p>
                <p className="text-xs text-slate-400 mt-1">{`Reason: ${cellData.anomalyReason}`}</p>
            </div>
          )}
        </div>
      );
  ...
  return (
    <div role="figure" aria-label="Chart of cell voltages and internal resistance. Use tab key to navigate through individual cells and view details.">
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          <XAxis dataKey="id" tick={{ fill: '#94a3b8' }} stroke="#475569" />
          <YAxis 
              yAxisId="left"
              dataKey="voltage"
              ...
              orientation="left"
              label={{ value: 'Voltage (V)', angle: -90, position: 'insideLeft', fill: '#94a3b8', dy: 40 }}
          />
          <YAxis 
              yAxisId="right"
              ...
              orientation="right"
              label={{ value: 'Resistance (mΩ)', angle: 90, position: 'insideRight', fill: '#94a3b8', dy: -60 }}
          />
          <Tooltip content={<CustomTooltip />} cursor={{fill: 'rgba(71, 85, 105, 0.3)'}} />
          <Legend verticalAlign="top" wrapperStyle={{paddingBottom: '10px'}} />
          <Bar dataKey="voltage" yAxisId="left" name="Voltage" onClick={(data) => onCellClick(data.id)}>
            {cells.map((entry, index) => (
              <RechartsCell 
                key={`cell-${index}`} 
                fill={entry.isAnomaly ? '#ef4444' : '#22d3ee'} 
