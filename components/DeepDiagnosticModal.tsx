...
const DeepDiagnosticModal: React.FC<DeepDiagnosticModalProps> = ({ isOpen, onClose, vehicleName }) => {
  ...
  return (
    ...
    >
      <div 
        className="bg-gray-900 border border-gray-800 rounded-lg shadow-2xl w-full max-w-md p-6 text-white"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-fuchsia-400">Deep Diagnostic (Tier 2)</h2>
        ...
        <p className="text-sm text-gray-400 mb-6">Simulating advanced analysis for <span className="font-semibold text-gray-300">{vehicleName}</span>...</p>
        ...
        {currentStep < simulationSteps.length ? (
                ...
                {simulationSteps.map((step, index) => (
                    <div key={index} className="flex items-center space-x-3 text-sm">
                        {index < currentStep ? (
                            ...
                            <div className="w-5 h-5 bg-gray-800 rounded-full flex-shrink-0" />
                        )}
                        <span className={`transition-colors ${index <= currentStep ? 'text-gray-300' : 'text-gray-500'}`}>
                            {step.text}
                        </span>
                    </div>
                ))}
            ...
            <div className="animate-fade-in">
                ...
                <div className="bg-black/50 p-4 rounded-lg space-y-3 border border-gray-800">
                    <div className="flex justify-between text-lg">
                        <span className="text-gray-400">Predicted SOH:</span>
                        <span className="font-bold text-white">{results?.soh}</span>
                    </div>
                    <div className="flex justify-between text-lg">
                        <span className="text-gray-400">Predicted RUL:</span>
                        <span className="font-bold text-white">{results?.rul}</span>
                    </div>
                    <div className="flex justify-between text-sm pt-2 border-t border-gray-800">
                        ...
                        <span className="font-semibold text-gray-300">{results?.confidence}</span>
                    </div>
                </div>
                <button 
                    onClick={onClose}
